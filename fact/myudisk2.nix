...

{
  ...
  config = mkIf config.services.myudisks2.enable {
    ...
    environment.etc = (mapAttrs' (name: value: nameValuePair "udisks2/${name}" { source = value; } ) configFiles) // {
      ...
      # warnings
      "libblockdev/conf.d/00-default.cfg".source = "${pkgs.libblockdev}/etc/libblockdev/3/conf.d/00-default.cfg";
      "libblockdev/conf.d/10-lvm-dbus.cfg".source = "${pkgs.libblockdev}/etc/libblockdev/3/conf.d/10-lvm-dbus.cfg";
    };
